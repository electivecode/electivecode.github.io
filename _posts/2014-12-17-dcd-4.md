...
I have an answer for you shortly.
...
{% capture table %}<table>
  <tr>
    ...
    <th><code>script</code> after <code>p</code></th>
  </tr>
  <tr>
...
1. It loads and executes `jquery-2.1.1.min.js`.
1. It loads and executes `app.js`, *but there's no DOM object* for `p` tag yet, so `$("p")` doesn't work, and `console.log` prints `undefined`.
1. It reads the `p` tag and creates a DOM object for it. *But it's too LATE!*

### What if you use `$(function() {});`

However, even when HTML is like this (DOM object for `p` tag doesn't exist when `app.js` is executed):
...
$(function() {
  ...
  // AFTER all DOM objects are created.
});
```
...
<div class="callout callout-info">
  ...
  <p>
    Code inside <code>$(function() {});</code> is executed <em>after</em> all DOM objects are created.
  </p>
</div>
{% endcapture %}
{{ callout4_1 }}
...
1. It reads the `p` tag and *creates a DOM object for it*.
1. Now that all DOM objects are created, it executes what's inside `$(function() {});` on `app.js`.
...
However, if you're writing a JavaScript library *for someone else*, you can't rely that he/she will put it after all the other tags. If you don't use `$(function(){});` in your library and the user of your library puts `<script>` inside `<head>`, your library will not work. That's not good.

So, it's a good practice to always put your code inside `$(function(){});` so that the user of your code can put `<script>` anywhere.
...
<div class="callout callout-info">
  ...
  <p>
    By always putting your code inside <code>$(function() {});</code>, users of your code can put the <code>script</code> anywhere.
  </p>
</div>

## What you now know

So, you now know everything about this code:
...
1. When all DOM objects on the page are created, then
1. Run `console.log($("p").html());`, which:
    1. Gets jQuery object for the `p` tag, and
    1. Prints out HTML code inside the `p` tag.

And remember that jQuery objects contain the corresponding DOM object.

Congratulations, that's it for today's lesson.
